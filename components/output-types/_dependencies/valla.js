  ...
  window.requestIdle(() => {
    ...
    const getUrlLandingAuth = () => {
      return arcEnv === 'prod'
        ...
        : `/pf/auth-fia/?_website=${arcSite}&outputType=subscriptions`
    }

    const getUrlSignwallHash = () => {
      return arcEnv === 'prod'
        ? `/signwall/?outputType=subscriptions&reloginHash=1`
        : `/signwall/?_website=${arcSite}&outputType=subscriptions&reloginHash=1`
    }
    ...
    if (iOS && getQuery('surface') === 'meter_limit_reached') {
      ...
      if (dataContentPremium !== 2 && dataContentPremium !== true) {
        ...
        window.ArcPOptions = {
          paywallFunction: function(campaign) {
            if (campaign.match(/signwallHard/) && !checkSession()) {
              ...
              if(signwall) signwall.className = 'signwall-modal open'
        ...
        script.src = `https://elcomercio-${arcSite}-${arcEnv}.cdn.arcpublishing.com/arc/subs/p.min.js?v=${getdata}` // al pasar por babel debe quedar asi https://elcomercio-${arcSite}-${arcEnv}.cdn.arcpublishing.com/arc/subs/p.min.js?v=${getdata}
        script.async = 'true'
