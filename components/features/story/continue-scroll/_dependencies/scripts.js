...
window.addEventListener('load', () => {requestIdle(() => {
  ...
  window.addEventListener( "message", function (event) {  
    ...
    if(origin === window.location.origin) {
      if(data.id === "story_iframe") {
        ...
        signwall.className = "signwall-modal open"
  ...
  const getTmpAd = () => {
      ...
      return tmpAdTargeting[1] ? '&tmp_ad=' + tmpAdTargeting[1] : ''
    };
  const tmpAdTarget = getTmpAd();
  const loadNextUrlStorage = (observedElement) => {
    ...
    if(nextStory.link){
      ...
      requestIdle(() => {
        ...
        let nextUrl = location.origin + "/carga-continua" + nextStory.link + "?ref=nota&ft=cargacontinua" + noTaboola + "&outputType=lite&nota=" + storyCounter + tmpAdTarget
        nextUrl = context()
