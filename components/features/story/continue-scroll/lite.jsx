...
window.addEventListener('load', () => {requestIdle(() => {
  ...
  const isPremiumUser = () => {
    ...
    if(window.localStorage && window.localStorage.hasOwnProperty('ArcId.USER_INFO') && window.localStorage.getItem('ArcId.USER_INFO') !== '{}'){
      ...
      if((COUNT_USER && COUNT_USER.sub.p.length) || window.isArcPUser)  { isPremium = true }
    }
    return isPremium;
  }
  ...
  if(!isPremiumUser() && contentTier && contentTier.getAttribute('content') === "locked") {
    return;
  } else {
