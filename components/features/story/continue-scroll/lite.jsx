...
window.addEventListener('load', () => {requestIdle(() => {
  ...
  const hasPaywall = () => {
    // Si existe popUp de signwall
    const wall = document.getElementById("signwall-app")
    // Si no tiene clase, quiere decir que viene de primera nota
    return wall && !wall.className.includes("signwall-app")
  }

  const loadNextUrlStorage = (observedElement) => {
    if(hasPaywall()) return null
