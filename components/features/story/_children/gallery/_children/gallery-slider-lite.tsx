...
const StoryHeaderChildGallerySlider: FC<FeatureProps> = (props) => {
  const { promoItems } = props

  const contentElements = promoItems?.basic_gallery?.content_elements

  const totalSlides = contentElements?.length || 0
  ...
  return slides.length > 0 ? (
    <section className={classes.elementsSlider} id="story-galery">
      <div
        ...
        <ul id="galery-ul" style={sliderStyle} className={classes.content}>
          {slides.map(({ _id: id, url, caption, subtitle }, i) => (
            <li key={`gallery-s-${id}`} style={slideStyle}>
      ...
      <script dangerouslySetInnerHTML={{ __html: handleGallery }} />
    </section>
  ) : null
}
