...
interface Props {
  ...
  subTitle: string
  isPremium: boolean
  contentElementsListOne: ContentElement
}

const StoryChildrenTitle: React.FC<Props> = (props) => {
  const {
    arcSite,
    primarySectionLink,
    primarySection,
  ...
  const { items, type } = contentElementsListOne
...
export default StoryChildrenTitle
