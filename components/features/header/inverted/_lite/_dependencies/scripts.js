...
/* document.addEventListener('DOMContentLoaded', () => {
  ...
  window.requestIdle(() => {
    setTimeout(() => {
      ...
      console.log('execute PIANO 🎹 ===>>>>> ', window.aid)
    }, 1000)

    document
      .getElementById('btn-identity-arc')
      .addEventListener('click', () => {
        window.tp = window.tp || []
        window.tp.push([
          'init',
...
export const pianoScript =
  '"use strict";document.addEventListener("DOMContentLoaded",function(){!function(e){var n=document.createElement("script");n.type="text/javascript",n.async=!0,n.src="https://sandbox.tinypass.com/xbuilder/experience/load?aid=PeVZORGJsu";var t=document.getElementsByTagName("script")[0];t.parentNode.insertBefore(n,t)}(),window.requestIdle(function(){setTimeout(function(){window.tp.experience.execute(),console.log("execute PIANO 🎹 ===>>>>> ",window.aid)},1e3),document.getElementById("btn-identity-arc").addEventListener("click",function(){window.tp=window.tp||[],window.tp.push(["init",function(){window.tp.pianoId.show({disableSignUp:!1,displayMode:"modal",screen:"login",containerSelector:"#login-form",loggedIn:function(e){console.log("user ",e.user," logged in with token",e.token)},loggedOut:function(){console.log("user logged out")}})}])})})});'
