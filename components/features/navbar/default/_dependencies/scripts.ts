...
export const signwallScript = (arcSite: ArcSite, arcEnv: string) =>
  '"use strict";const arcSite="<<arcSite>>",arcEnv="<<arcEnv>>";document.addEventListener("DOMContentLoaded",function(){const e=e=>{window.dataLayer=window.dataLayer||[];const t={event:"tag_signwall",eventCategory:"Web_Sign_Wall_General",eventAction:e};window.dataLayer.push(t)};window.requestIdle(()=>{const t=JSON.parse(window.localStorage.getItem("ArcId.USER_PROFILE")),{firstName:n="",lastName:i="",uuid:a=""}=t||{};document.querySelector(".nav-d__sign").addEventListener("click",()=>{a?(e("web_swg_link_ingresaperfil"),window.location.href=`/mi-perfil/?_website=${arcSite}&outputType=subscriptions`):(e("web_swg_link_ingresacuenta"),window.location.href=`/signwall/?_website=${arcSite}&outputType=subscriptions&signwallOrganic=1`)}),a&&document.querySelector(".nav-d__sign").classList.add("signed")})});'
