    ...
    fetch(`${API_BASE}usuario/${USUARIO}/partidos?v=${new Date().getTime()}`)
      ...
      .then((data) => {
        if (typeof data.mensaje !== 'undefined' && data.mensaje === 'ok') {
          ...
          fetch(
            `${API_BASE}usuario/${ANONIMO}/partidos?v=${new Date().getTime()}`
          )
            .then((response) => response.json())
            .then((data2) => {
              if (
                typeof data2.mensaje !== 'undefined' &&
                data2.mensaje === 'ok'
              ) {
                setValidLoad(true)
    ...
    <div className={classes.grid}>
      ...
      {matchs &&
        matchs.map((match) => {
          ...
          if (
            (match.estado === 0 || match.estado === 1) &&
            closeMatchs.includes(match.id)
          ) {
            matchStatus = 2
          }
              ...
              <>
                {title === true && (
                  <>
                    ...
                    <h2 className="journeyTitle">Fecha {match.jornada}</h2>
                  </>
                )}
                <MatchBox
                  key={match.id}
