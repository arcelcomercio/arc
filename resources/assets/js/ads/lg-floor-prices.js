;(function() {
  ...
  if (refCtrl) {
    ...
    function time1Delay(time) {
      time1 = time
    }
    function time2Delay(time) {
      time2 = time
    }
  ...
  googletag.cmd.push(function() {
    ...
    if (slots.length == 0) {
      googletag.pubads().addEventListener('slotAdded', function(ev) {
        let size =
          ev.slot.getSizes()[0]['l'] + ', ' + ev.slot.getSizes()[0]['j']
        if (size != '1, 1') {
          slots.push(ev.slot)
        }
      })
  ...
  function ayos() {
    ...
    var publisherHour =
      PublisherTimeZone - userContext_timezone() + date.getHours()
    req['publisherTime'] =
      publisherHour + ':' + date.getMinutes() + ':' + date.getSeconds()
    req['day'] = date.getDay()
    req['url'] = URL
    ...
    function sendReq() {
      ...
      if (request.readyState == 4 && request.status != 200) {
        ...
        if (deb == true) {
          ...
          let timeStamp = Math.round(time - startTime)
          console.log(
            'AYOS: ERROR ' + request.status + ' - ' + timeStamp + 'ms'
          )
        }
        unload()
    ...
    googletag.cmd.push(function() {
      ...
      for (let i in slots) {
        ...
        if (isNaN(slots[i].getSizes()[0]['l'])) {
          size =
            slots[i].getSizes()[1]['l'] + ', ' + slots[i].getSizes()[1]['j']
        } else {
          size =
            slots[i].getSizes()[0]['l'] + ', ' + slots[i].getSizes()[0]['j']
        }
      ...
      googletag.cmd.push(function() {
        googletag.pubads().addEventListener('slotAdded', function(ev) {
          ...
          if (isNaN(ev.slot.getSizes()[0]['l'])) {
            size =
              ev.slot.getSizes()[1]['l'] + ', ' + ev.slot.getSizes()[1]['j']
          } else {
            size =
              ev.slot.getSizes()[0]['l'] + ', ' + ev.slot.getSizes()[0]['j']
          }
    ...
    googletag.pubads().addEventListener('slotRenderEnded', function(ev) {
      ...
      if (size != '1, 1') {
        if (deb == true) {
          ...
          let timeStamp = Math.round(time - startTime)
          console.log(
            'AYOS: Sending Result - ' + req2['adID'] + ' - ' + timeStamp + 'ms'
          )
        }
        ...
        request.onreadystatechange = function() {
          if (request.readyState == 4 && request.status == 200) {
            ...
            if (deb == true) {
              ...
              let timeStamp = Math.round(time - startTime)
              console.log(
                'AYOS: Result Sent - ' + req2['adID'] + ' - ' + timeStamp + 'ms'
              )
            }
          }
        }
        ...
        if (deb == true) {
          ...
          let timeStamp = Math.round(time - startTime)
          console.log(
            'AYOS: ' + req2['adID'] + ' - Rendered time: ' + timeStamp + 'ms'
          )
        }
        if (logLevel >= 2) {
          log(4)
